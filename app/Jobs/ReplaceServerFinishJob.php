...
/**
 * The replacement droplet's ping came back clean — re-applies the OLD
 * server's PasarGuard node + WireGuard profile onto it, then asks the user
 * to confirm deleting the old one. The old server is left completely alone
...
class ReplaceServerFinishJob implements ShouldQueue
{
    ...
    public function handle(Nutgram $bot, PasarguardNodeInstaller $installer): void
    {
        ...
        try {
            $result = $installer->install($this->newIp, 'root', $newSecret->root_password, $this->wireguardProfileId);
