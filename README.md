...
از `/start` → «⚙️ تنظیمات» → «🔒 وایرگاردها» می‌توانید چند کانفیگ وایرگارد (فرمت wg-quick، شامل `[Interface]` و `[Peer]`) با نام دلخواه ذخیره کنید. اسمی که می‌دهید همان اینترفیس/فایل روی سرور می‌شود (مثلاً `it` → `/etc/wireguard/it.conf`). همه‌ی این کانفیگ‌ها موقع نود کردن هر سروری خودکار روی آن هم فعال می‌شوند، و از دکمه‌ی «🔄 بروزرسانی وایرگاردها» در اطلاعات سرور می‌توانید بدون نصب دوباره‌ی کل نود، فقط وایرگاردها را با آخرین تنظیمات همگام کنید.
